    ...
    #    os.system('pkill -f firefox')
        ...
        if tmpAssignment['Video']:
                ...
                for val in tmpAssignment['URLS']:
                        ...
                        urls+=val
                        urls+='" '
                command = 'export DISPLAY=:0 && /usr/bin/chromium-browser --kiosk --new-window --start-fullscreen --disable-session-crashed-bubble --disable-infobars --window-size=1920,1080 --window-position=0,0 --disable-restore-session-state --disable-features=SignIn --incognito '+(urls)
                subprocess.Popen(command, shell=True)
